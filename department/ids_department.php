...
include("../connect.php");
 ...
 $data = "SELECT * FROM id_f_request  INNER JOIN users ON users.id = id_f_request.users_id 
 INNER JOIN company ON id_f_request.company_id = company.id WHERE hod_prove = 'none'";
$total_f = mysqli_query($conn, $data);


$date = "SELECT COUNT(*) AS total_u FROM id_f_request WHERE hod_prove = 'none' ";
...
<header class="head">
    ...
    <center>
        <h1 class="hd">OITS ID's DEPARTMENT</h1>
        <div  id="notification" >
      ...
      <table>
        ...
        <?php
          ...
          echo "<td>".$all['hod_prove']."</td>";
                   ...
                   echo "<td><a href=\"department_action.php?sign_ids=$all[ids_id]\" onClick=\"return confirm('Approving this Application will Automatically Attach your Email to the Request, Do you want to continue ?')\">Approve</a></td>";
                  }
                  ?>
                  </table>
                </div>
